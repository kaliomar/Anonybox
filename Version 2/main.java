...
public class main {
	...
	public static void main(String[] args) {
		try {
			// SSD = Server Side Device
			// CSD = Client Side Device
			String type = JOptionPane.showInputDialog("Are you type SSD[Type number 1] or CSD[Type number 2]");
			if (type.equals("1")) {
				ssd();
	...
	public static void ssd() {
		//Server Side Device
		try {
		String keyy = getSaltString();
		String userr = getSaltString();
		String passs = getSaltString();
    	JOptionPane.showMessageDialog(null,"Make the client connect using your IP\n"
    			+ "Encryption Key: "+keyy+"\n"
    					+ "Username: "+userr+"\n"
    					+ "Password: "+passs+"\n"
    					+ "Type the creds as commands.\n"
    					+ "The server will start after you press OK");
		String port = "1111";
		@SuppressWarnings("resource")
		ServerSocket server = new ServerSocket(Integer.parseInt(port));
	    while(true) {
			Socket c=server.accept();
			Thread t = new main.t(c,keyy,userr,passs); 
			t.start();
		}
		}catch(Exception e) {
			e.printStackTrace();
